...
router.delete('/cancelar/:id', authMiddleware, isAluno, async (req, res) => {
  try {
    ...
    if (reservaResult.rows.length === 0) {
      return res.status(404).json({ message: 'Reserva não encontrada ou não pode ser cancelada' });
    }
...
router.put('/renovar/:id', authMiddleware, isAluno, async (req, res) => {
  try {
    ...
    if (reservaResult.rows.length === 0) {
      return res.status(404).json({ message: 'Empréstimo não encontrado ou não pode ser renovado' });
    }
    ...
    if (hoje > dataVencimento) {
      return res.status(400).json({ message: 'Não é possível renovar um empréstimo em atraso' });
    }
