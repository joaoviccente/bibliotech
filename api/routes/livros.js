...
router.patch('/concluir/:id_reserva', authMiddleware, isAluno, async (req, res) => {
  try {
    ...
    if (!reserva) {
      return res.status(404).json({ message: 'Reserva não encontrada ou já foi processada' });
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      res.json({ message: 'Livro marcado como concluído com sucesso! Aguarde a devolução física para que o livro seja liberado.' });
